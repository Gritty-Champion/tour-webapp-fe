...
export const HolidayTour = ({ data, getnamePackages, setTotalAmount }) => {
  console.log(data);
  ...
  useEffect(() => {
    if (data == null || !data) {
      ...
      // Check if uniqueValue exists in dammyDataSingleBus
      foundData = dammyDataSingleBus.find((item) => item.uniqueId === data);

      // If not found in dammyDataSingleBus, check in doublePackages
      if (!foundData) {
        foundData = dammyDataDoubleBus.find((item) => item.uniqueId === data);
      }

      // If still not found, check in iconicPackages
      if (!foundData) {
        foundData = dammyDataIconic.find((item) => item.uniqueId === data);
      }
